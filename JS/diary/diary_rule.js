$(function () {
  ...
  let bag_rule_lightbox =
    document.getElementsByClassName("bag_rule_lightbox")[0];

  //取得切換按鈕的部分
  let fa_pen_to_squaren =
    document.getElementsByClassName("fa-pen-to-square")[0];

  //到進入編輯按鈕時,燈箱不見
  //取得第一個關閉的元素
  let use_close = document
    .getElementsByClassName("use")[0]
    .querySelector("input");
  ...
  function addArticle(articlID) {
    $.ajax({
      ...
      success: (response) => {
        ...
        // 與上一篇文章為不同篇，加上內容
        create_list_el.insertAdjacentHTML(
          "afterbegin",
          `
